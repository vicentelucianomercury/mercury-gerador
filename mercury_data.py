...
def diagnosticar_cotas(filepath=QUANTUM_FILE) -> str | None:
    """Quando NENHUMA série é lida, tenta explicar o porquê em PT claro.
    Retorna uma mensagem, ou None se não souber dizer.
    Só é chamada no caminho de falha do app — pode abrir o arquivo em modo
    fórmula (mais pesado) sem impacto no fluxo normal."""
    try:
        wb = openpyxl.load_workbook(filepath, data_only=False)
    except Exception as e:
        return f"não consegui abrir o arquivo como Excel (.xlsx): {e}"
    ws = wb.active
    for name in wb.sheetnames:
        if "quantum" in name.lower():
            ws = wb[name]; break
    tem_qtlink = tem_formula = False
    n_valores = 0
    for row in ws.iter_rows(min_row=1, max_row=min(ws.max_row or 1, 60)):
        for cell in row:
            ...
            if getattr(cell, "data_type", None) == "f":
                tem_formula = True
                if isinstance(v, str) and "QTLINK" in v.upper():
                    tem_qtlink = True
            else:
                n_valores += 1
    wb.close()
    if tem_qtlink:
        return (
            "o arquivo contém fórmulas do **Quantum Link (QTLINK)** que não foram "
            "convertidas em valores — as cotas só existem quando o Excel atualiza o "
            "link ao vivo, e este arquivo foi salvo sem esses valores. "
            "No Excel: atualize o Quantum Link, aguarde as séries preencherem, então "
            "**cole como valores** (Ctrl+C na área com dados → Colar Especial → Valores) "
            "e salve. Ou exporte pela opção que já gera valores (o arquivo que funciona "
            "vem com as colunas Nome do Ativo | Data | Cota preenchidas).")
    if tem_formula and n_valores == 0:
        return ("o arquivo só tem fórmulas, sem valores em cache. Atualize/recalcule "
                "no Excel e salve novamente (idealmente colando como valores).")
    return None
