...
try:
    from mercury_data import (calcular_metricas, ultima_data_atualizacao,
                              cdi_acumulado, diagnosticar_cotas)
    ...
    with col1:
        ...
        if uploaded:
            ...
            try:
                ...
                met, dt = None, None
                st.error(f"Não consegui ler este arquivo de cotas: {e}")

            if met is not None:
                n = sum(1 for f in FUNDOS
                        if met.get(f["nome_quantum"],{}).get("ret_12m") is not None)
                _bench = {"CDI","IPCA","IMA-B","IBOVESPA","IBOV"}
                n_series = sum(1 for k in met if str(k).upper() not in _bench)

                if not met or dt is None:
                    # Nada parseado. Tenta explicar a causa específica (a mais
                    # comum: arquivo com fórmulas QTLINK não convertidas em valores).
                    motivo = None
                    try:
                        motivo = diagnosticar_cotas(fp)
                    except Exception:
                        pass
                    if motivo:
                        st.error("Este arquivo de cotas não pôde ser usado: " + motivo)
                    else:
                        st.error(
                            "Não encontrei nenhuma série com **datas válidas** neste "
                            "arquivo. Causas comuns: a coluna **Data** veio como texto, "
                            "a **aba** dos dados tem outro nome, ou as **colunas** estão "
                            "fora de ordem. No arquivo que funciona, a 1ª aba tem "
                            "**Nome do Ativo | Data | Cota/Preço** com a Data em formato "
                            "de data.")
                    st.session_state.quantum_file = None
                    st.session_state.metricas = None
                elif n == 0:
                    # Parseou séries, mas nenhuma casou com os fundos da matriz.
                    st.warning(
                        f"Li **{n_series} série(s)** deste arquivo, mas **nenhuma "
                        f"casou com os {len(FUNDOS)} fundos** da matriz em uso. "
                        "Provavelmente os nomes dos fundos aqui estão diferentes "
                        "dos da lista. Verifique os nomes (ou a matriz) e tente de novo.")
                    st.session_state.quantum_file = None
                    st.session_state.metricas = None
                else:
