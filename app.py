...
.fund-tbl{width:100%;border-collapse:collapse}
.fund-tbl th{font-size:10px;font-weight:500;color:#aaa;text-align:center;
  padding:0 8px 6px;border-bottom:1px solid #eee;text-transform:uppercase;
  letter-spacing:.04em}
.fund-tbl th:first-child{text-align:left}
.fund-tbl td{padding:8px 8px;font-size:12px;text-align:center;
  border-bottom:1px solid #f5f5f5;color:#444;vertical-align:middle}
.fund-tbl td:first-child{text-align:left;color:#333}
.fund-tbl tr:last-child td{border-bottom:none}
.ret-val{font-weight:600;color:#0a2744}
.vol-val{color:#666}
...
def badge_html(classe):
    bg, fg = CORES.get(classe, ("#eee","#333"))
    return (f'<span style="background:{bg};color:{fg};padding:3px 10px;'
            f'border-radius:99px;font-size:11px;font-weight:600">{classe}</span>')
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_sl:
        st.markdown('<div class="group-lbl">Renda Fixa</div>',
                    unsafe_allow_html=True)
        for c in CLASSES_RF:
            mn, mx = bandas[c]
        ...
        st.markdown('<div class="group-lbl">Outras classes</div>',
                    unsafe_allow_html=True)
    ...
    with col_sum:
        ...
        st.markdown(
            ...
            f'Renda Fixa (grupo) — {rf_t*100:.1f}%</div>',
            unsafe_allow_html=True)
        for c in CLASSES_RF:
            v = aloc_classe[c]
            if v>0:
        ...
        if avisos:
            for av in avisos:
                st.markdown(f'<div class="warn-box">⚠ {av}</div>',
                            unsafe_allow_html=True)
    ...
    for c in CLASSES:
        ...
        st.markdown(
            ...
            f'<span style="color:#555">{pct_c*100:.1f}%</span>'
        ...
        # Cabeçalho da tabela com as 5 colunas
        h_fundo, h_pct, h_valor, h_r6, h_r12, h_r24, h_vol = st.columns([3,1,1.2,1,1,1,1])
        for col, lbl in zip([h_fundo,h_pct,h_valor,h_r6,h_r12,h_r24,h_vol],
                             ["Fundo","% classe","Valor","Ret 6m","Ret 12m","Ret 24m","Vol 12m"]):
            with col:
        ...
        for f in fundos_c:
            ...
            cf, cp, cv, cr6, cr12, cr24, cvol = st.columns([3,1,1.2,1,1,1,1])
            with cf:
                st.markdown(f'<div style="font-size:12px;color:#333;'
                            f'padding:8px 0 4px">{nq[:48]}</div>',
                            unsafe_allow_html=True)
            with cp:
            ...
            with cv:
                ...
                st.markdown(f'<div style="font-size:11px;color:#666;'
                            f'padding:8px 0 4px;text-align:right">'
                            f'{fmt_brl(vf)}</div>', unsafe_allow_html=True)
            for col_r, val_r in [(cr6,r6),(cr12,r12),(cr24,r24)]:
                with col_r:
                    st.markdown(f'<div style="font-size:12px;font-weight:600;'
                                f'color:#0a2744;padding:8px 0 4px;text-align:center">'
                                f'{fmt_pct(val_r)}</div>', unsafe_allow_html=True)
            with cvol:
                st.markdown(f'<div style="font-size:11px;color:#666;'
                            f'padding:8px 0 4px;text-align:center">'
    ...
    for c in CLASSES:
        ...
        if fundos_c:
            for f in fundos_c:
                ...
                m = metricas.get(nq,{})
                rows.append({
                    "Classe":c, "Fundo":nq[:55],
                    "% Total":f"{p_fd*pct_c*100:.1f}%",
                    "Valor":fmt_brl(val_f),
            ...
            rows.append({"Classe":c,"Fundo":"(sem fundo aprovado)",
                        "% Total":f"{pct_c*100:.1f}%","Valor":fmt_brl(val_c),
                        "Ret 6m":"—","Ret 12m":"—","Ret 24m":"—","Vol 12m":"—"})
    ...
    with cp:
        try:
            ...
            st.download_button("⬇ Baixar PPT", ppt,
                               ...
                               use_container_width=True)
        except: st.info("PPT em construção")
    with cd:
        try:
        ...
        except: st.info("PDF em construção")
    with cn:
